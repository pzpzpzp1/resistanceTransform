...
assert(norm(dists)==0);
[ridx, dists] = knnsearch(HMesh.V2P, boundaryVertPairs(measurements,:), 'K', 1);
assert(norm(dists)==0);
...
for measurementInd = 1:numel(measurements)
    ...
    if debugging
        ...
        scatter3(HMesh.V2P(stinds,1),HMesh.V2P(stinds,2),HMesh.V2P(stinds,3),200,[1 0 0; 0 1 0],'filled');
    end
end
